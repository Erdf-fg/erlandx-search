...
# CORS Middleware - Allow ALL for debugging connectivity issues
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # Allow Vercel, Localhost, etc explicitly
...
async def startup_event():
    global engine
    print("🚀 App Startup: Initializing Search Engine...")
    try:
        engine = SearchEngine()
        print("✅ Search Engine Online")
    except Exception as e:
        print(f"❌ Critical Error loading engine: {e}")
        # We don't exit here so the app can at least respond with error status
...
def root():
    if not engine:
        return {"status": "starting", "message": "Engine is initializing..."}
