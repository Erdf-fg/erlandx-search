...
if [ ! -z "$HF_URL" ]; then
    ...
    echo "🔑 Masukkan ACCESS TOKEN Hugging Face baru anda (Input akan disembunyikan):"
    echo "   (Buat token di: https://huggingface.co/settings/tokens)"
    read -s HF_TOKEN
    
    if [ -z "$HF_TOKEN" ]; then
        echo "⚠️ Token kosong. Menggunakan metode manual (mungkin akan pakai credential lama)."
        git remote remove hf 2> /dev/null
        git remote add hf "$HF_URL"
    else
        # Extract Username from URL (assumes format https://huggingface.co/spaces/USERNAME/REPO)
        HF_USER=$(echo "$HF_URL" | cut -d'/' -f5)
        
        # Construct Authenticated URL: https://USER:TOKEN@huggingface.co/...
        # Remove 'https://' first
        CLEAN_URL="${HF_URL/https:\/\//}"
        AUTH_URL="https://$HF_USER:$HF_TOKEN@$CLEAN_URL"
        
        git remote remove hf 2> /dev/null
        git remote add hf "$AUTH_URL"
        echo "✅ Credential baru diterapkan untuk user: $HF_USER"
    fi
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "❌ Gagal push ke Hugging Face. Pastikan Token 'Write' access dan URL benar."
    fi
fi
